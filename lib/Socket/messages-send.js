...
const makeMessagesSocket = (config) => {
   ...
   **/
   ...
   const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, useUserDevicesCache, useCachedGroupMetadata, statusJidList, additionalNodes }) => {
      ...
      if (!isNewsletter && buttonType && buttonType !== 'buttons') {
   ...
   } else if ('buttonsMessage' in content) {
   const msg = await Utils_1.generateWAMessageFromContent(jid, {
      buttonsMessage: content.buttonsMessage
   }, { quoted: options.quoted })

   return await relayMessage(jid, msg.message, {
      messageId: msg.key.id
   })
} else if ('carouselMessage' in content) {
   const { caption = '', footer = '', cards = [] } = content.carouselMessage

   const carouselCards = await Promise.all(cards.map(async (card) => ({
      header: WAProto_1.proto.Message.InteractiveMessage.Header.create({
         title: card.headerTitle || '',
         subtitle: card.headerSubtitle || '',
         productMessage: {
            product: {
               productImage: (await Utils_1.prepareWAMessageMedia({ image: { url: card.imageUrl } }, { upload: waUploadToServer })).imageMessage,
   ...
   return await relayMessage(jid, msg.message, {
      messageId: msg.key.id
        ...
        return await relayMessage(jid, msg.message, {
         messageId: msg.key.id,
         ...(typeof getParticipantAttr === 'function' ? getParticipantAttr() : {})
      })
    } else if ('productMessage' in content) {
      const {
         title,
         description,
         thumbnail,
      ...
      // bagian asli untuk semua pesan lainnya
      let mediaHandle
